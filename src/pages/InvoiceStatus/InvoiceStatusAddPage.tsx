...
export default function InvoiceStatusAddPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      setError("");
      await InvoiceStatusService.create({ statusName, description });
      setSuccess("Invoice status created successfully!");
      setTimeout(() => {
        navigate("/invoice-statuses");
      }, 2000);
    } catch (err) {
      setError("Failed to create invoice status");
    } finally {
      setLoading(false);
    }
  };
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ mt: 3 }}>
          <TextField
            ...
            onChange={(e) => {
              ...
              setErrors((prev) => ({
                ...prev,
                statusName: validateName(e.target.value),
              }));
            }}
            margin="normal"
            required

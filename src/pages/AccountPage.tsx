...
export default function AccountPage() {
	...
	const validateNewPassword = (pwd: string) => {
		...
		setNewPwdError(validatePassword(pwd));
	};

	const handleSubmitChangePassword = () => {
		const error = validatePasswordChange(currentPwd, newPwd, confirmPwd);
		if (error) {
	...
	const submitChangePassword = async () => {
		if (!handleSubmitChangePassword()) return;
		try {
	...
	const save = async () => {
		...
		try {
			await UsersService.updateEmail(authUser.id.toString(), editedEmail);
	...
	return (
		<Container
			maxWidth="md"
			sx={{ mt: 10, mb: 6, backgroundColor: "f5f5f5" }}
		>
			<Paper
				sx={{ p: 4 }}
				elevation={2}
